<template>
  <div class="fill-body" id="flapp-surveys">
    <main class="app-content">
      ...
      <family-law v-show="getCurrentStepIndex() == 2" v-bind:step="getStep(2)"></family-law>
      <case-management v-show="getCurrentStepIndex() == 3" v-bind:step="getStep(3)"></case-management>
      <parenting-priority v-show="getCurrentStepIndex() == 4" v-bind:step="getStep(4)"></parenting-priority>
      <child-relocation v-show="getCurrentStepIndex() == 5" v-bind:step="getStep(5)"></child-relocation>
      <enforcement-agree v-show="getCurrentStepIndex() == 6" v-bind:step="getStep(6)"></enforcement-agree>
      <print v-show="getCurrentStepIndex() == 7" v-bind:step="getStep(7)"></print>
    </main>
  </div>
</template>
