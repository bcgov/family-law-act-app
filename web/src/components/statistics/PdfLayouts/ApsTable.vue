<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Affidavit of Personal service' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{apsInfo[0].total}} </span>
        </div>
...
@Component
export default class ApsTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    apsInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.apsInfo = [this.results.application_details.APS];
    }
