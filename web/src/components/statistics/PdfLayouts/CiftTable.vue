<template>
    <div v-if="dataReady" class="print-block">
        <div 
            ...
            Consent to an Informal Trial (Kamloops only): 
        </div>
        <b-table            
            :items="ciftInfo"
            :fields="fields"
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Consent to an Informal Trial (Kamloops only)' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{ciftInfo[0].total}} </span>
        </div>
...
@Component
export default class CiftTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    ciftInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.ciftInfo = [this.results.application_details.CIFT];
    }
