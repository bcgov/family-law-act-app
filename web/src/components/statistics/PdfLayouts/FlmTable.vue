<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Family Law Matter' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{flmInfo[0].total}} </span>
        </div>        
    </div>
</template>
...
@Component
export default class FlmTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false;
    flmInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){       
       this.flmInfo = [this.results.application_details.FLM];
    }   
