<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Financial Statement' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{fsInfo[0].total}} </span>
        </div>
...
@Component
export default class FsTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    fsInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.fsInfo = [this.results.application_details.FS];
    }
