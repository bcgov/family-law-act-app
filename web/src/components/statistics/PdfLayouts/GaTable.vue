<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Guardianship Affidavit' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{gaInfo[0].total}} </span>
        </div>
...
@Component
export default class GaTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    gaInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.gaInfo = [this.results.application_details.GA];
    }
