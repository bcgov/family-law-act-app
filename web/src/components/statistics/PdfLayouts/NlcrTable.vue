<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Notice of Removal of Lawyer for Child' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{nlcrInfo[0].total}} </span>
        </div>
...
@Component
export default class NlcrTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    nlcrInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.nlcrInfo = [this.results.application_details.NLCR];
    }
