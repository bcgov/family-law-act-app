<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Notice of Removal of Lawyer for Party' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{nlprInfo[0].total}} </span>
        </div>
...
@Component
export default class NlprTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    nlprInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.nlprInfo = [this.results.application_details.NLPR];
    }
