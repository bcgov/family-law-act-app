<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Order – General' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{ordInfo[0].total}} </span>
        </div>
...
@Component
export default class OrdTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    ordInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.ordInfo = [this.results.application_details.ORD];
    }
