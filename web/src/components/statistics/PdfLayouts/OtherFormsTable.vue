<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Other Forms' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{forminfo[0].total}} </span>
        </div>
...
@Component
export default class OtherFormsTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    forminfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.forminfo = [this.results.application_details.OTHER];
    }
