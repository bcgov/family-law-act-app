<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Protection Order' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{poInfo[0].total}} </span>
        </div>        
    </div>
</template>
...
@Component
export default class PoTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;    
       
    dataReady = false; 
    poInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.poInfo = [this.results.application_details.PO];       
    } 
}
</script>