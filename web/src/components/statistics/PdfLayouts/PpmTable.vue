<template>
    <div v-if="dataReady" class="print-block">
        <div 
            ...
            Priority Parenting Matter Applications: 
        </div>
        <b-table            
            :items="ppmInfo"
            :fields="fields"
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Priority Parenting Matter' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{ppmInfo[0].total}} </span>
        </div>
...
@Component
export default class PpmTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    ppmInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.ppmInfo = [this.results.application_details.PPM];
    }
