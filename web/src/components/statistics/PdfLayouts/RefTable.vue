<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Referral Request' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{refInfo[0].total}} </span>
        </div>
...
@Component
export default class RefTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    refInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.refInfo = [this.results.application_details.REF];
    }
