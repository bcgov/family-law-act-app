<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Request for Service of Family Protection Order' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{rpsInfo[0].total}} </span>
        </div>
...
@Component
export default class RpsTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    rpsInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.rpsInfo = [this.results.application_details.RPS];
    }
