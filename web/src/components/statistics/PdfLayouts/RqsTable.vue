<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Request for Scheduling' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{rqsInfo[0].total}} </span>
        </div>
...
@Component
export default class RqsTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    rqsInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.rqsInfo = [this.results.application_details.RQS];
    }
