<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Standalone' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{stndInfo[0].total}} </span>
        </div>
...
@Component
export default class StndTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    stndInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.stndInfo = [this.results.application_details.STND];
    }
