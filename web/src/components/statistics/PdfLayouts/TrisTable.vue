<template>
    <div v-if="dataReady" class="print-block">
        ...
        <div style="font-size:9pt; margin-bottom: 0.5rem;">
            <b>Total Number of 'Trial Readiness Statement' Applications:</b> <span style="font-size:11pt;" class="text-primary">{{trisInfo[0].total}} </span>
        </div>
...
@Component
export default class TrisTable extends Vue {

    @Prop({required:true})
    results!: reportInfoType;
       
    dataReady = false; 
    trisInfo: formReportInfoType[] = [];
    ...
    public extractInfo(){
       this.trisInfo = [this.results.application_details.TRIS];
    }
