<template>
    <div v-if="dataReady">
        <b-card id="print" style="border:1px solid; border-radius:5px;" bg-variant="white" class="mt-4 mb-4 container" no-body>
            <form-45-layout v-bind:result="result"/>
        </b-card>
    </div>
</template>
...
import Form45Layout from "./Form45Layout.vue"
...
export default class Form45 extends Vue {
    ...
    mounted(){
        ...
        this.result = this.getAFFResultData();       
        this.dataReady = true;
    ...
    public onPrint() { 
        ...
        const pdf_type = Vue.filter('getPathwayPdfType')("affidavit")
        const pdf_name = "affidavit";       
        const el= document.getElementById("print");

        const applicationId = this.$store.state.Application.id;
        const bottomLeftText = `"PFA 762    `+moment().format("MM/YYYY")+` \\a           Form 45";`;
        ...
        .then(res => {
            ...
            this.UpdatePathwayCompleted({pathway:"affidavit", isCompleted:true});
