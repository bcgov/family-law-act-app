...
export default class PreviewFormsApsEfsp extends Vue {
    ...
    mounted(){
        ...
        const apsPageNumber = this.stPgNo.APS.PreviewFormsAPS;
        
        let nonCompleteForm = false;
        let nonCompleteFormPage = 0;
        
        const pageForm = this.$store.state.Application.steps[this.stPgNo.APS._StepNo].pages[apsPageNumber]
        if(pageForm.active && pageForm.progress !=100){
            nonCompleteForm = true;
            nonCompleteFormPage = apsPageNumber            
        }

        if(nonCompleteForm){
            this.$store.commit("Application/setCurrentStepPage", {currentStep: this.stPgNo.APS._StepNo, currentPage: nonCompleteFormPage});
        }
        else if(this.checkErrorOnPages([this.stPgNo.COMMON._StepNo, this.stPgNo.APS._StepNo])) 
            this.dataReady = true;
