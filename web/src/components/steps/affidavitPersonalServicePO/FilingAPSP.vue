...
export default class FilingApsp extends Vue {
    ...
    public determinePages(){
        ...
        if (this.survey.data?.sworn && this.steps[this.stPgNo.OTHER._StepNo].result?.otherFormsSurvey?.data?.filingMethod){
            ...
            togglePages([this.stPgNo.APSP.ElectronicFilingStatementAPSP], this.survey.data.sworn == 'y' && eFiling, this.currentStep);
        }
    ...
    public reloadPageInformation() {
        ...
        if (this.step.result?.filingApspSurvey) {            
            this.survey.data = this.step.result.filingApspSurvey.data;
    ...
    public setEfsDocumentList(){
        ...
        if (this.step.result.aboutAffiantApspSurvey?.data?.ApplicantName){
            const applicantName = this.step.result.aboutAffiantApspSurvey.data.ApplicantName
    ...
    beforeDestroy() {   
        ...
        this.UpdateStepResultData({step:this.step, data: {filingApspSurvey: Vue.filter('getSurveyResults')(this.survey, this.currentStep, this.currentPage)}})
    }
}
</script>
