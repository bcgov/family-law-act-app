...
export default class PreviewFormsApspEfsp extends Vue {
    ...
    mounted(){
        ...
        const apspPageNumber = this.stPgNo.APSP.PreviewFormsAPSP;
        
        let nonCompleteForm = false;
        let nonCompleteFormPage = 0;
        
        const pageForm = this.$store.state.Application.steps[this.stPgNo.APSP._StepNo].pages[apspPageNumber]
        ...
        if(nonCompleteForm){
            this.$store.commit("Application/setCurrentStepPage", {currentStep: this.stPgNo.APSP._StepNo, currentPage: nonCompleteFormPage});
        }
        else if(this.checkErrorOnPages([this.stPgNo.COMMON._StepNo, this.stPgNo.APSP._StepNo])) 
            this.dataReady = true;
        
        if(this.checkErrorOnPages([this.stPgNo.OTHER._StepNo, this.stPgNo.APSP._StepNo])) this.dataReady = true;
        window.scrollTo(0, 0);
    }   
