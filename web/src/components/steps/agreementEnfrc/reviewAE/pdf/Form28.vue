...
<div v-if="dataReady">    
    ...
    <b-card id="print" style="border:1px solid; border-radius:5px;" bg-variant="white" class="mt-4 mb-4 container" no-body>
        <form-28-layout v-bind:result="result"/>
    </b-card>
</div>
</template>
...
import Form28Layout from "./Form28Layout.vue";
...
export default class Form28 extends Vue {
    ...
    public onPrint() { 
        const pdf_type = Vue.filter('getPathwayPdfType')("agreementEnfrc28")
        const pdf_name = "request-to-file-an-order"
        const el= document.getElementById("print");

        const applicationId = this.$store.state.Application.id;
        const bottomLeftText = `"PFA745    `+moment().format("MMMM D, YYYY")+` \\a           Form 28";`;
    ...
    public getEnfrcResultData() {         
        ...
        Vue.filter('extractRequiredDocuments')(result, 'agreementEnfrc28')
