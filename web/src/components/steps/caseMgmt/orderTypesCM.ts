...
export function getOrderTypeCM(form: string, toggle = false, toggleNext = false) {
    ...
    const p = stPgNo.CM
    const orders = {
        'changeServiceRequirement':{turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.ChangingOrCancellingAServiceOrNotice,p.ApplicationUnderFOAEAA],                              text:'Changing or cancelling the requirement for service or notice to a person, including allowing another method for the service of a document'},
        'changeRequirement':       {turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.ChangingOrCancellingAnyOtherRequirement,p.ApplicationUnderFOAEAA],                           text:'Changing or cancelling `any other requirement` under the rules, including a time limit'},
        'remoteAttendance':        {turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.AttendanceUsingElectronicCommunication,p.ApplicationUnderFOAEAA],                            text:'Attending at a court appearance using another method of attendance, including in-person, by telephone, video or other electronic means'},
        ...
        'otherProvinceOrder':      {turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.RecognizingAnOrderFromOutsideBc, p.ContactInformationOtherParty,p.ApplicationUnderFOAEAA],   text:'Recognizing an order from outside British Columbia (other than a support order)'},
        ...
        'section242':              {turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.RequiringAccessToInformation,p.ApplicationUnderFOAEAA],                                      text:'Requiring access to information in accordance with section 242 of the Family Law Act'},
        'section12':               {turquoise: true,  pages:[p.WithoutNoticeOrAttendance,p.ApplicationUnderFOAEAA],                                                                      text:"Authorizing an official of the court to make an application under section 12 of the <a href='https://laws-lois.justice.gc.ca/eng/acts/f-1.4/page-1.html#h-221520' target='_blank'>Family Orders and Agreements Enforcement Assistance Act (Canada)</a> for the release of information for the establishment or variation of a support provision or the enforcement of a family provision"}
    }
