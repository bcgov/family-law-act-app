...
export function getOrderTypeCM(form: string, toggle = false, toggleNext = false) {
    ...
    const orders = {
        ...
        'remoteAttendance':        {turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.AttendanceUsingElectronicCommunication],  text:'Attending at a court appearance by telephone, video or other electronic means'},
        ...
        'otherProvinceOrder':      {turquoise: true,  pages:[p.WithoutNoticeOrAttendance, p.RecognizingAnOrderFromOutsideBc],         text:'Recognizing an order from outside British Columbia (other than a support order)'},
