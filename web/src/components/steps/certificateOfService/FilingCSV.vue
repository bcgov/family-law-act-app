...
export default class FilingCsv extends Vue {
    ...
    public determinePages(){
        ...
        if (this.survey.data?.sworn && this.steps[this.stPgNo.OTHER._StepNo].result?.otherFormsSurvey?.data?.filingMethod){
            ...
            togglePages([this.stPgNo.CSV.ElectronicFilingStatementCSV], this.survey.data.sworn == 'y' && eFiling, this.currentStep);
        }
    ...
    public reloadPageInformation() {
        ...
        if (this.step.result?.filingCsvSurvey) {            
            this.survey.data = this.step.result.filingCsvSurvey.data;
    ...
    beforeDestroy() {   
        ...
        this.UpdateStepResultData({step:this.step, data: {filingCsvSurvey: Vue.filter('getSurveyResults')(this.survey, this.currentStep, this.currentPage)}})
    }
}
</script>
