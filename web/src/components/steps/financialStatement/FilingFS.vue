...
import surveyJson from "./forms/filling-affidavit-fs.json";
...
export default class FilingFs extends Vue {
    ...
    public determinePages(){
        ...
        if (this.survey.data?.sworn && this.steps[this.stPgNo.OTHER._StepNo].result?.otherFormsSurvey?.data?.filingMethod){
            ...
            togglePages([this.stPgNo.FS.ElectronicFilingStatementFS], this.survey.data.sworn == 'y' && eFiling, this.currentStep);
        }
    ...
    public reloadPageInformation() {
        ...
        if (this.step.result?.filingFsSurvey) {            
            this.survey.data = this.step.result.filingFsSurvey.data;
    ...
    public setEfsDocumentList(){
        ...
        if (this.step.result.aboutAffiantFsSurvey?.data?.ApplicantName){
            const applicantName = this.step.result.aboutAffiantFsSurvey.data.ApplicantName
            efsDocumentList.push({documentName: 'Financial Statement', efsApplicantName: applicantName});
        }
              
       this.UpdateCommonStepResults({data:{'fsEfsDocuments':efsDocumentList}});
    }
    ...
    beforeDestroy() {   
        ...
        this.UpdateStepResultData({step:this.step, data: {filingFsSurvey: Vue.filter('getSurveyResults')(this.survey, this.currentStep, this.currentPage)}})
    }
}
</script>
