...
export default class PreviewFormsFsEfsp extends Vue {
    ...
    mounted(){
        ...
        const csvPageNumber = this.stPgNo.FS.PreviewFormsFS;
        
        let nonCompleteForm = false;
        let nonCompleteFormPage = 0;
        
        const pageForm = this.$store.state.Application.steps[this.stPgNo.FS._StepNo].pages[csvPageNumber]
        ...
        if(nonCompleteForm){
            this.$store.commit("Application/setCurrentStepPage", {currentStep: this.stPgNo.FS._StepNo, currentPage: nonCompleteFormPage});
        }
        else if(this.checkErrorOnPages([this.stPgNo.COMMON._StepNo, this.stPgNo.FS._StepNo])) 
            this.dataReady = true;
        
        if(this.checkErrorOnPages([this.stPgNo.OTHER._StepNo, this.stPgNo.FS._StepNo])) this.dataReady = true;
        window.scrollTo(0, 0);
    }   
