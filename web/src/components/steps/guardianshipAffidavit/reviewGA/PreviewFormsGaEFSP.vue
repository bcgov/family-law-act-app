...
export default class PreviewFormsGaEfsp extends Vue {
    ...
    mounted(){
        ...
        if(nonCompleteForm){
            this.$store.commit("Application/setCurrentStepPage", {currentStep: this.stPgNo.GA._StepNo, currentPage: nonCompleteFormPage});
        }
        else if(this.checkErrorOnPages([this.stPgNo.COMMON._StepNo, this.stPgNo.GA._StepNo])) 
            this.dataReady = true;
        
        if(this.checkErrorOnPages([this.stPgNo.OTHER._StepNo, this.stPgNo.GA._StepNo])) this.dataReady = true;
        window.scrollTo(0, 0);
    }   
