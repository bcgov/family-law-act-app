<template>
    <div v-if="dataReady">
        <b-card id="print" style="border:1px solid; border-radius:5px;" bg-variant="white" class="mt-4 mb-4 container" no-body>
            <form-41-layout v-bind:result="result"/>
        </b-card>
    </div>
</template>
...
import Form41Layout from "./Form41Layout.vue"
...
export default class Form41 extends Vue {
    ...
    mounted(){
        ...
        this.result = this.getNlcrResultData();       
        this.dataReady = true;
    ...
    public onPrint() { 
        ...
        const pdf_type = Vue.filter('getPathwayPdfType')("noticeRemoveLawyerChild")
        const pdf_name = "notice-of-removal-of-lawyer-for-child";       
        ...
        .then(res => {
            ...
            this.UpdatePathwayCompleted({pathway:"noticeRemoveLawyerChild", isCompleted:true});
    ...
    public getNlcrResultData() {         
        
        const result = Object.assign({},this.$store.state.Application.steps[0].result); 
        for(const stepIndex of [this.stPgNo.OTHER._StepNo, this.stPgNo.NLCR._StepNo]){
