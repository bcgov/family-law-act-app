...
import surveyJson from "./forms/trial-readiness-statement.json";
...
export default class TrialReadinessStatement extends Vue {
    ...
    public reloadPageInformation() {
        ...
        if (this.step.result?.trialReadinessStatementSurvey) {            
    ...
    public setPages() {
        ...
        if (this.survey.data) {
            ...
            const canContinue = !(surveyResponses.Scheduled == 'n' && surveyResponses.FileTris == 'n' );
            this.disableNextButton = !canContinue;

            if (!canContinue){
                trialReadinessStatementAllPages.push(p.PreviewFormsTRIS)
            }            
            togglePages(trialReadinessStatementAllPages, canContinue, this.currentStep);            
        }
    }    
